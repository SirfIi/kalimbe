 ...
 <div class="alice-bg padding-top-70 padding-bottom-70">
            ...
            <div class="job-listing-container">
              ...
              <div id="job-filter-result" class="job-filter-result">
                  ...
                  <div class="job-list">
                    ...
                    <div class="body">
                      <div class="content">
                        <h4><a href="{{url('jobdetail', $j->id)}}">{{$j->titre}}</a></h4>
                        <div class="info">
                      ...
                      <div class="more"><a href="{{url('jobdetail', $j->id)}}">
                        <span>Date Limite:</span>
                        <p class="deadline">{{ date_format(date_create($j->date_exp),"d-m-Y")}}</p></a>
                      </div>
                    </div> 
                  </div>
                  @endforeach
              ...
              <div class="job-filter-wrapper">
                <div class="job-filter-dropdown same-pad category">
                  <select onchange="search()" id ="secteur" class="selectpicker">
                ...
                <div class="job-filter-dropdown same-pad location">
                  <select onchange="search()" id ="localisation" class="selectpicker">
                ...
                <div class="job-filter-dropdown same-pad location">
                  <select onchange="search()" id ="type_contrat" class="selectpicker">
                      <option value="" selected>Type de Contrat</option>
                      <option  value="CDD Temps plein" class="autre">CDD Temps plein</option> 
                      <option  value="CDD Temps partiel" class="autre">CDD Temps partiel</option>
                      <option  value="CDI Temps plein" class="autre">CDI Temps plein</option>
                      <option  value="CDI Temps partiel" class="autre">CDI Temps partiel</option> 
                  </select>
                </div>
                <div class="job-filter-dropdown same-pad location">
                ...
                <div class="job-filter-dropdown same-pad location">
                  <select onchange="search()" id ="date" class="selectpicker">
                      <option value="" selected>Date de Publication</option>
                      <option  value="3600" class="autre">Moins de 1 heure</option> 
                      <option  value="86400" class="autre">Il y'a 24 heures</option>
                      <option  value="259200" class="autre">Il y'a 3 Jours</option>
                      <option  value="604800" class="autre">Il y'a 7 Jours</option>
                      <option  value="2592000" class="autre">1 mois et +</option> 
                  </select>
                </div>
              </div>
            </div>
    ...
    console.log(jobs.data);
     ...
     function search(){
       res = []
       var s = secteur.value;
       var l = localisation.value; 
       var e = experience.value; 
       var t = type_contrat.value; 
       ...
       var exit = false;
        ...
        for(var i=0; i<data.length; i++){
          ...
          var time_ = ( new Date().getTime() - new Date(data[i].created_at).getTime())/1000
          console.log(time_);
          console.log(d);

          if( (d >= time_) ||(d < time_ && d == 2592000)){
            exit = false;
            for(var j=0; j<res.length; j++){
              if(res[j].id == data[i].id){
                exit = true;
      ...
      if(res.length>0){
        ...
        result.innerHTML ='';
        for(var j=0; j<res.length; j++){
          result.innerHTML +='<div class="job-list"><div class="thumb"><a href="#"><img class="image" src="/images/'+res[j].user.photoUrl+'" onerror=this.src="images/briefcase.png";></a></div><div class="body"><div class="content"><h4><a href="/jobdetail/'+res[j].id+'">'+res[j].titre+'</a></h4><div class="info"><span class="company"><a href="#"><i data-feather="briefcase"></i>'+res[j].user.name+'</a></span><span class="office-location"><a href="#"><i data-feather="map-pin"></i>'+res[j].localisation+'</a></span><span class="full-time"><a href="#"><i data-feather="clock"></i>'+res[j].type_contrat+'</a></span> </div></div><div class="more"><a href="/jobdetail/'+res[j].id+'"><span>Date Limite:</span><p class="deadline">'+res[j].date_exp+'</p></a></div></div> </div>';
        }
      }
    }
